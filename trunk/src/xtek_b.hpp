					...
					const int nz_voxels)
{
  ...
  real start[3], end[3];
#pragma omp parallel shared(det_y, det_z, angles, grid_offset, voxel_size) private(curr_angle, curr_ray_y, curr_ray_z, cos_curr_angle, sin_curr_angle, start, end, ray_offset), firstprivate(source_x, source_y, source_z, det_x, n_rays_y, n_rays_z, n_angles, ray_data, vol_data, nx_voxels, ny_voxels, nz_voxels)
  {
    int nz_offset = 0;
    int nz_step = 0;
