...
/*********WITH PREVIOUS STEP************/
bp::list initialize_cgls(np::ndarray ndarray_pixels,
	np::ndarray ndarray_angles,
	double rotation_centre, int resolution,
	int niterations, int nthreads,
	CCPi::algorithms alg,
...
bp::list reconstruct_iter(np::ndarray ndarray_pixels,
	np::ndarray ndarray_angles,
	double rotation_centre, int resolution,
	int niterations, int nthreads,
	CCPi::algorithms alg,
	bp::list last_iteration ,
	const real regularize = 0.0,
	np::ndarray norm = np::zeros(bp::make_tuple(0), np::dtype::get_builtin<float>()), bool is_pixels_in_log = true
	)
{
	...
	*/
	np::ndarray last_iteration_volume = bp::extract<np::ndarray>(last_iteration[0]);
