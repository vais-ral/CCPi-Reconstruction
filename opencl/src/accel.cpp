...
void machine::run_cone_ah(const char name[], dev_ptr pix_buf, dev_ptr vox_buf,
			  dev_ptr xy0_buff, dev_ptr xy1_buff,
			  dev_ptr xy_offsets, dev_ptr h, dev_ptr lengths,
			  const int nv, const int nz, const int start,
			  ...
			  const int device, std::vector<event_t> *events)
{
  if (size > max_work0 or dim3 > max_work1)
	...
	status = kernel.setArg(1, vox_buf);
      if (status == CL_SUCCESS)
	status = kernel.setArg(2, xy0_buff);
      if (status == CL_SUCCESS)
	status = kernel.setArg(3, xy1_buff);
      if (status == CL_SUCCESS)
	status = kernel.setArg(4, xy_offsets);
      if (status == CL_SUCCESS)
	...
	status = kernel.setArg(9, start);
      if (status == CL_SUCCESS)
	status = kernel.setArg(10, xy_size);
      if (status == CL_SUCCESS)
	status = kernel.setArg(11, pzbz);
      if (status == CL_SUCCESS)
	status = kernel.setArg(12, midp);
      if (status == CL_SUCCESS)
