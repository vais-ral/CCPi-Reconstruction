...
#pragma omp parallel shared(h_pixels, v_pixels, pixels, voxels, angles, vox_size, vox_origin, mapping) firstprivate(n_angles, nh_pixels, nv_pixels, nx, ny, nz, map_type)
  {
	  ...
	  for (int ax = 0; ax < a_step; ax++) {
		...
		if (nwork > 0) {
		  if (nwork > 2 * max_n + 1)
		    ...
		    calc_xy_z(&(pixels[a][h][0]), ij_arr, l_xy, nwork,
...
#pragma omp parallel shared(h_pixels, v_pixels, pixels, voxels, angles, vox_size, vox_origin, mapping) firstprivate(n_angles, nh_pixels, nv_pixels, nx, ny, nz,map_type) num_threads(nthreads)
	...
	*/
	...
	sl_int mem_size = (accel_proj * sl_int(nh_pixels))
	  * (sl_int(nv_pixels) * sizeof(pixel_type)
	...
	while (mem_size > machine::available_mem(thread_id) or
	  ...
	  if (n_angles % factor != 0)
	    accel_proj++;
	  mem_size = (accel_proj * sl_int(nh_pixels))
	    * (sl_int(nv_pixels) * sizeof(pixel_type)
	       + sl_int(ah_size) * (sizeof(recon_type) + sizeof(int)))
	    + sl_int(x_block) * sl_int(y_block) * sl_int(nz)
	    * sizeof(voxel_type);
